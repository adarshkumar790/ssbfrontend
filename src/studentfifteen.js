...
const Studentfifteen = () => {
  const [students, setStudents] = useState([]);
  
  useEffect(() => {
    axios.get('http://localhost:8081/studentfifteen/')
      .then(res => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete('http://localhost:8081/studentfifteen/' + id);
      setStudents(prevStudents => prevStudents.filter(student => student.ID !== id)); // Update state without reloading
    } catch (err) {
      console.log(err);
    }
  }
    ...
    <div className='d-flex vh-100 bg-danger justify-content-center align-items-center'>
      <div className='w-100 bg-success rounded p-3 shadow'>
        <div className="d-flex justify-content-between align-items-center mb-3"> {/* Align to the right */}
          <h3 className="text-center">Session 2015-2017</h3> {/* Center the session header */}
          <Link to="/createfifteen" className='btn btn-dark'>Add Student Details</Link>
        </div>
                    ...
                    <tr key={i}>
                      ...
                      <td>
                        <div className="btn-group" role="group">
                          <Link to={`studentforteen/${data.ID}`} className='btn btn-primary'>Update</Link>
                          <button className='btn btn-danger ml-3' onClick={() => handleDelete(data.ID)}>Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))
