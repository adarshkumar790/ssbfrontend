...
const StudentEighteen = () => {
    const [students, setStudents] = useState([]);
   
    useEffect (()=>{
      axios.get('http://localhost:8081/studenteighteen/')
    ...
    const handleDelete = async(id) => {
       try {
        await axios.delete('http://localhost:8081/studenteighteen/'+id)
    ...
    return (
        <div className='d-flex vh-100 bg-danger justify-content-center align-items-center'>
            ...
            <div className="d-flex justify-content-between align-items-center mb-3"> {/* Align to the right */}
                    <h3 className="text-center">Session 2018-2020</h3> {/* Center the session header */}
                    <Link to="/createeighteen" className='btn btn-dark'>Add Student Details</Link>
                </div>
                                    ...
                                    <tr key={i}>
                                        ...
                                        <td>
                                            <div className="btn-group" role="group">
                                                <Link to={`studenteighteen/${data.ID}`} className='btn btn-primary'>Update</Link>
...
export default StudentEighteen;
