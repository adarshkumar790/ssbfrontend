...
const StudentForteen = () => {
    const [students, setStudents] = useState([]);
   
    useEffect (()=>{
      axios.get('http://localhost:8081/studentforteen/')
    ...
    const handleDelete = async(id) => {
       try {
        await axios.delete('http://localhost:8081/studentforteen/'+id)
    ...
    return (
        <div className='d-flex vh-100 bg-danger justify-content-center align-items-center'>
            ...
            <div className="d-flex justify-content-between align-items-center mb-3"> {/* Align to the right */}
                    <h3 className="text-center">Session 2014-2015</h3> {/* Center the session header */}
                    <Link to="/createforteen" className='btn btn-dark'>Add Student Details</Link>
                </div>
